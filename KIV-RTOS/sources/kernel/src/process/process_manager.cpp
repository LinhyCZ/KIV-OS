...
void CProcess_Manager::Handle_Memory_SWI(NSWI_Memory_Service svc_idx, uint32_t r0, uint32_t r1, uint32_t r2, TSWI_Result& target)
{
    if (!mCurrent_Task_Node)
        return;
    ...
    switch (svc_idx)
    {
        case NSWI_Memory_Service::sbrk:
            target.r0 = sUserspaceMem.sbrk(mCurrent_Task_Node->task, r0);
            break;
        //Memory Free might be placed here
    }
}
