...
uint32_t CUART::Read(char* buf, uint32_t len) {
    return uartBuffer.read(buf, len);
}

/**
 * Tries to read input from the UART.
 * 
 * If no data are available, then char value of 255 is returned.
 */
char CUART::Read_From_Register() {
    uint32_t c = 255;   
...
{
    char c = sUART0.Read_From_Register();
